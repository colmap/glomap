...
double CalcAngle(const Rigid3d& pose1, const Rigid3d& pose2) {
    double cos_r = ((pose1.rotation.inverse() * pose2.rotation).toRotationMatrix().trace() - 1) / 2;
...
double CalcTransAngle(const Rigid3d& pose1, const Rigid3d& pose2) {
    double cos_r = (pose1.translation).dot(pose2.translation) / (pose1.translation.norm() * pose2.translation.norm());
    cos_r = std::min(std::max(cos_r, -1.), 1.);
...
Eigen::Matrix3d AngleAxisToRotation(const Eigen::Vector3d& aa_vec) {
    ...
    if (aa_norm > EPS) {
        return Eigen::AngleAxis<double>(aa_norm, aa_vec.normalized()).toRotationMatrix();
    } else {
        Eigen::Matrix3d R;
        R(0, 0) = 1;
        R(1, 0) = aa_vec[2];
