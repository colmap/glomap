...
boost::no_property, boost::property<boost::edge_weight_t, double> >      weighted_graph;
...
int BFS(const std::vector<std::vector<int>>& graph,
        int root, std::vector<int>& parents,
    ...
    while (!q.empty()) {
        ...
        for (int neighbor : graph[current_vertex]) {
            if (std::find(banned_edges.begin(), banned_edges.end(), std::make_pair(current_vertex, neighbor)) != banned_edges.end())
                continue;
            if (std::find(banned_edges.begin(), banned_edges.end(), std::make_pair(neighbor, current_vertex)) != banned_edges.end())
                continue;
...
image_t MaximumSpanningTree(const ViewGraph& view_graph,
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        // Set the weight to be negative, then the result would be a maximum spanning tree
