...
bool CheckCheirality(const Rigid3d &pose, const Eigen::Vector3d &x1, const Eigen::Vector3d &x2, double min_depth, double max_depth) {
...
double GetOrientationSignum(const Eigen::Matrix3d& F, const Eigen::Vector3d& epipole, const Eigen::Vector2d& pt1, const Eigen::Vector2d& pt2) {
...
void EssentialFromMotion(const Rigid3d &pose, Eigen::Matrix3d *E) {
    *E << 0.0, -pose.translation(2), pose.translation(1), pose.translation(2), 0.0, -pose.translation(0), -pose.translation(1), pose.translation(0), 0.0;
    *E = (*E) * pose.rotation.toRotationMatrix();
}

// Get the essential matrix from relative pose
void FundamentalFromMotionAndCameras(const Camera& camera1, const Camera& camera2, const Rigid3d &pose, Eigen::Matrix3d *F) {
    Eigen::Matrix3d E;
...
double HomographyError(const Eigen::Matrix3d& H, const Eigen::Vector2d& x1, const Eigen::Vector2d& x2) {
