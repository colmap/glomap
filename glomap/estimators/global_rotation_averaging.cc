...
bool RotationEstimator::EstimateRotations(const ViewGraph& view_graph,
    ...
    for (auto &[image_id, image] : images) {
        ...
        if (options_.use_gravity && image.gravity_info.has_gravity) {
            image.cam_from_world.rotation = Eigen::Quaterniond(AngleToRotUp(rotation_estimated_[image_id_to_idx_[image_id]]));
        } else {
            image.cam_from_world.rotation = Eigen::Quaterniond(AngleAxisToRotation(rotation_estimated_.segment(image_id_to_idx_[image_id], 3)));
        }
    }
...
void RotationEstimator::SetupLinearSystem(const ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images) {
    ...
    rotation_estimated_.resize(3 * images.size()); // allocate more memory than needed
    image_t num_dof = 0;
    image_t num_img = 0;
    for (auto &[image_id, image] : images) {
        if (!image.is_registered)
        ...
        if (options_.use_gravity && image.gravity_info.has_gravity) {
            rotation_estimated_[num_dof] = RotUpToAngle(image.gravity_info.R_align.transpose() * image.cam_from_world.rotation.toRotationMatrix());
            num_dof++;

            if (fixed_camera_id_ == -1) {
                fixed_camera_rotation_ = Eigen::Vector3d(0, RotUpToAngle(image.gravity_info.R_align.transpose() * image.cam_from_world.rotation.toRotationMatrix()), 0);
                fixed_camera_id_ = image_id;
            }
        }
        else {
            rotation_estimated_.segment(num_dof, 3) = Rigid3dToAngleAxis(image.cam_from_world);
            num_dof += 3;
        }
        num_img++;
    }
    ...
    if (options_.verbose)
        std::cout << "num_img: " << image_id_to_idx_.size() << ", num_dof: " << num_dof << std::endl;
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        rel_temp_info_[pair_id].R_rel = image_pair.cam2_from_cam1.rotation.toRotationMatrix();

        // Align the relative rotation to the gravity
        if (options_.use_gravity) {
            if (images[image_id1].gravity_info.has_gravity) {
                rel_temp_info_[pair_id].R_rel = rel_temp_info_[pair_id].R_rel * images[image_id1].gravity_info.R_align;
            }

            if (images[image_id2].gravity_info.has_gravity) {
                rel_temp_info_[pair_id].R_rel = images[image_id2].gravity_info.R_align.transpose() * rel_temp_info_[pair_id].R_rel;
            }
        }

        if (options_.use_gravity && images[image_id1].gravity_info.has_gravity && images[image_id2].gravity_info.has_gravity) {
            Eigen::Vector3d aa = RotationToAngleAxis(rel_temp_info_[pair_id].R_rel);
            double error = aa[0] * aa[0] + aa[2] * aa[2];

            // Keep track of the error for x and z axis for gravity-aligned relative pose
    ...
    for (const auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        if (rel_temp_info_[pair_id].has_gravity) {
            ...
            if (!options_.use_gravity || !images[image_id1].gravity_info.has_gravity) {
                for (int i = 0; i < 3; i++) {
                    coeffs.emplace_back(Eigen::Triplet<double>(curr_pos + i, vector_idx1 + i, -1));
                }
            } else 
                // else, other components are zero, and can be safely ignored
                coeffs.emplace_back(Eigen::Triplet<double>(curr_pos + 1, vector_idx1, -1));

            // Similarly for the second componenet
            if (!options_.use_gravity || !images[image_id2].gravity_info.has_gravity) {
                for (int i = 0; i < 3; i++) {
                    coeffs.emplace_back(Eigen::Triplet<double>(curr_pos + i, vector_idx2 + i, 1));
                }
            } else
                coeffs.emplace_back(Eigen::Triplet<double>(curr_pos + 1, vector_idx2, 1));
    ...
    if (options_.use_gravity && images[fixed_camera_id_].gravity_info.has_gravity) {
        coeffs.emplace_back(Eigen::Triplet<double>(curr_pos, image_id_to_idx_[fixed_camera_id_], 1));
        curr_pos++;
    } else {
        for (int i = 0; i < 3; i++) {
            coeffs.emplace_back(Eigen::Triplet<double>(curr_pos + i, image_id_to_idx_[fixed_camera_id_] + i, 1));
        }
        curr_pos += 3;
    }
...
bool RotationEstimator::SolveL1Regression(const ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images) {
    L1SolverOptions opt_l1_solver;
    opt_l1_solver.max_num_iterations = 10;

    L1Solver<Eigen::SparseMatrix<double>> l1_solver(opt_l1_solver, sparse_matrix_);
    double last_norm = 0;
    double curr_norm = 0; 

    ComputeResiduals(view_graph, images);
    if (options_.verbose)
        std::cout << "ComputeResiduals done " << std::endl;

    int iteration = 0;
    for (iteration = 0; iteration < options_.max_num_l1_iterations; iteration++) {
        if (options_.verbose)
            std::cout << "L1 ADMM iteration: " << iteration << std::endl;
        ...
        if (options_.verbose)
            std::cout << "residual:" << (sparse_matrix_ * tangent_space_step_ - tangent_space_residual_).array().abs().sum() << std::endl;
        ...
        if (ComputeAverageStepSize(images) < options_.l1_step_convergence_threshold || std::abs(last_norm - curr_norm) < EPS) {
            if (std::abs(last_norm - curr_norm) < EPS)
                std::cout << "std::abs(last_norm - curr_norm) < EPS" << std::endl;
            iteration++;
            break;
        }
        opt_l1_solver.max_num_iterations = std::min(opt_l1_solver.max_num_iterations * 2, 100);
    }
    if (options_.verbose)
        std::cout << "L1 ADMM total iteration: " << iteration << std::endl;
    return true;
...
bool RotationEstimator::SolveIRLS(const ViewGraph& view_graph,
    ...
    if (options_.verbose)
        std::cout << "sigma: " << options_.irls_loss_parameter_sigma << std::endl;
    ...
    ComputeResiduals(view_graph, images);
    int iteration = 0;
    for (iteration = 0; iteration < options_.max_num_irls_iterations; iteration++) {
        if (options_.verbose)
            std::cout << "IRLS iteration: " << iteration << std::endl;
        ...
        for (auto &[pair_id, pair_info] : rel_temp_info_) {
            ...
            if (pair_info.has_gravity)
                err_squared = std::pow(tangent_space_residual_[image_pair_pos], 2) + pair_info.xz_error;
            // Otherwise, we consider all 3 dof
            else
                err_squared = tangent_space_residual_.segment<3>(image_pair_pos).squaredNorm();
            ...
            if (std::isnan(w)) {
                std::cout << "nan weight!" << std::endl;
                return false;
            }

            // If both cameras have gravity, then only 1 equation
            if (pair_info.has_gravity)
                weights_irls[image_pair_pos] = w;
        ...
        // Check the residual. If it is small, stop
        if (ComputeAverageStepSize(images) < options_.irls_step_convergence_threshold) {
            iteration++;
            break;
        }
    }
    if (options_.verbose)
        std::cout << "IRLS total iteration: " << iteration << std::endl;

    return true;
}

void RotationEstimator::UpdateGlobalRotations(const ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images) {
    for (const auto &[image_id, image] : images) {
        ...
        if (!(options_.use_gravity && image.gravity_info.has_gravity)) {
            Eigen::Matrix3d R_ori = AngleAxisToRotation(rotation_estimated_.segment(vector_idx, 3));

            rotation_estimated_.segment(vector_idx, 3)
                = RotationToAngleAxis(R_ori * AngleAxisToRotation(-tangent_space_step_.segment(vector_idx, 3)));
        } else {
...
void RotationEstimator::ComputeResiduals(const ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images) {
    int curr_pos = 0;
    for (auto &[pair_id, pair_info]: rel_temp_info_) {
        ...
        if (pair_info.has_gravity) {
            tangent_space_residual_[pair_info.index] = (RelAngleError(pair_info.angle_rel, rotation_estimated_[image_id_to_idx_[image_id1]], rotation_estimated_[image_id_to_idx_[image_id2]]));
        } else {
            Eigen::Matrix3d R_1, R_2;
            if (options_.use_gravity && images[image_id1].gravity_info.has_gravity) {
                R_1 = AngleToRotUp(rotation_estimated_[image_id_to_idx_[image_id1]]);
            } else {
                R_1 = AngleAxisToRotation(rotation_estimated_.segment(image_id_to_idx_[image_id1], 3));
            }

            if (options_.use_gravity && images[image_id2].gravity_info.has_gravity) {
                R_2 = AngleToRotUp(rotation_estimated_[image_id_to_idx_[image_id2]]);
            } else {
                R_2 = AngleAxisToRotation(rotation_estimated_.segment(image_id_to_idx_[image_id2], 3));
            }

            tangent_space_residual_.segment(pair_info.index, 3) = -RotationToAngleAxis(R_2.transpose() * pair_info.R_rel * R_1);
        }
    }

    if (options_.use_gravity && images[fixed_camera_id_].gravity_info.has_gravity)
        tangent_space_residual_[tangent_space_residual_.size() - 1] = rotation_estimated_[image_id_to_idx_[fixed_camera_id_]] - fixed_camera_rotation_[1];
    else 
        tangent_space_residual_.segment(tangent_space_residual_.size() - 3, 3)
            = RotationToAngleAxis(
                AngleAxisToRotation(fixed_camera_rotation_).transpose()
                * AngleAxisToRotation(rotation_estimated_.segment(image_id_to_idx_[fixed_camera_id_], 3)));
}

double RotationEstimator::ComputeAverageStepSize(const std::unordered_map<image_t, Image>& images) {
    ...
    for (const auto &[image_id, image] : images) {
        ...
        if (options_.use_gravity && image.gravity_info.has_gravity) {
            ...
            total_update += tangent_space_step_.segment(image_id_to_idx_[image_id], 3).norm();
        }
    }
