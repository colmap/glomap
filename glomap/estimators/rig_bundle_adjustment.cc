...
bool RigBundleAdjuster::Solve(const ViewGraph& view_graph,
  ...
  // Reset the problem
  Reset(camera_rigs, images);

  // Add the constraints that the point tracks impose on the problem
  AddPointToCameraConstraints(view_graph, camera_rigs, cameras, images, tracks);
...
void RigBundleAdjuster::Reset(const std::vector<CameraRig>& camera_rigs,
                              std::unordered_map<image_t, Image>& images) {
  ...
  for (auto& [track_id, track] : tracks) {
    ...
    for (const auto& observation : tracks[track_id].observations) {
      ...
      if (image_id_to_camera_rig_index_.find(observation.first) ==
        ...
        image_t idx_rig = image_id_to_camera_rig_index_[observation.first];
        const Rigid3d& cam_from_rig =
            camera_rigs[idx_rig].CamFromRig(camera_id);
        ...
        problem_->AddResidualBlock(
            cost_function,
            loss_function_.get(),
            image_id_to_rig_from_world_[observation.first]
                ->rotation.coeffs()
                .data(),
  ...
  for (auto& rigs : rigs_from_world_) {
    for (auto& rig : rigs) {
      if (problem_->HasParameterBlock(rig.translation.data())) {
        ...
        parameter_ordering->AddElementToGroup(rig.rotation.coeffs().data(), 1);
      }
    }
  }
