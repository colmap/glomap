...
Eigen::Vector3d RandVector3d(std::mt19937& random_generator, double low, double high) {
...
bool GlobalPositioner::Solve(const ViewGraph& view_graph,
    ...
    // Parameterize the variables, set image poses / tracks / scales to be constant if desired
    ...
    if (options_.constraint_type == GlobalPositionerOptions::POINTS_AND_CAMERAS_BALANCED) {
...
void GlobalPositioner::InitializeRandomPositions(
    ...
    if (options_.verbose)
        std::cout << "Constrained positions: " << constrained_positions.size() << std::endl;
}
...
void GlobalPositioner::AddCameraToCameraConstraints(
    ...
    for (const auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        if (images.find(image_id1) == images.end() || images.find(image_id2) == images.end())
            continue;

        track_t counter = scales_.size();
        scales_.insert(std::make_pair(counter, 1));

        Eigen::Vector3d translation = -images[image_id2].cam_from_world.Derotate(image_pair.cam2_from_cam1.translation);
        ceres::CostFunction *cost_function = BATAPairwiseDirectionError::Create(translation);
    ...
    if (options_.verbose)
        std::cout << problem_->NumResidualBlocks() << " camera to camera constraints were added to the position estimation problem." << std::endl;
}
...
void GlobalPositioner::AddPointToCameraConstraints(
    ...
    if (options_.verbose)
        std::cout << num_pt_to_cam << " point to camera constriants were added to the position estimation problem." << std::endl;
    ...
    if (num_cam_to_cam > 0 && options_.constraint_type == GlobalPositionerOptions::POINTS_AND_CAMERAS_BALANCED) {
        weight_scale_pt =
            options_.constraint_reweight_scale *
            static_cast<double>(num_cam_to_cam) /
            static_cast<double>(num_pt_to_cam);
    }
    if (options_.verbose)
        std::cout << "Point to camera weight scaled: " << weight_scale_pt << std::endl;
    
    if (loss_function_ptcam_uncalibrated_ == nullptr) {
        loss_function_ptcam_uncalibrated_ = new ceres::ScaledLoss(options_.loss_function, 0.5 * weight_scale_pt, ceres::DO_NOT_TAKE_OWNERSHIP);
    }

    if (options_.constraint_type == GlobalPositionerOptions::POINTS_AND_CAMERAS_BALANCED) {
        loss_function_ptcam_calibrated_ = new ceres::ScaledLoss(options_.loss_function, weight_scale_pt, ceres::DO_NOT_TAKE_OWNERSHIP);
    } else {
...
void GlobalPositioner::AddTrackToProblem(
    ...
    for (const auto &observation : tracks[track_id].observations) {
        ...
        Eigen::Vector3d translation = image.cam_from_world.Derotate(image.features_undist[observation.second]);
        ceres::CostFunction* cost_function = BATAPairwiseDirectionError::Create(translation);
        ...
        if (options_.generate_scales || !tracks[track_id].is_initialized) {
            ...
            Eigen::Vector3d trans_calc = tracks[track_id].xyz - image.cam_from_world.translation;
...
void GlobalPositioner::AddCamerasAndPointsToParameterGroups(
    ...
    for (auto& [image_id, image] : images) {
        if (problem_->HasParameterBlock(image.cam_from_world.translation.data())) {
            parameter_ordering->AddElementToGroup(image.cam_from_world.translation.data(), group_id);
        }
...
void GlobalPositioner::ParameterizeVariables(
    ...
    // For the global positioning, do not set any camera to be constant for easier convergence
...
void GlobalPositioner::ConvertResults(std::unordered_map<image_t, Image>& images) {
    // translation now stores the camera position, needs to convert back to translation
    for (auto &[image_id, image] : images) {
