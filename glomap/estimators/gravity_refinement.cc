...
void GravityRefiner::RefineGravity(const ViewGraph& view_graph,
  ...
  const std::unordered_map<image_pair_t, ImagePair>& image_pairs = view_graph.image_pairs;
  const std::unordered_map<image_t, std::unordered_set<image_t>>& adjacency_list = view_graph.GetAdjacencyList();
  ...
  for (auto image_id : error_prone_images) {
    if ((counter_progress + 1) % 10 == 0 || counter_progress == error_prone_images.size() - 1) {
  ...
  ceres::Problem problem(problem_options);
    ...
    for (const auto &neighbor : neighbors) {
      ...
      if (images.at(image_id1).gravity_info.has_gravity == false || images.at(image_id2).gravity_info.has_gravity == false)
          continue;
      
      if (image_id1 == image_id){
          gravities.emplace_back((image_pairs.at(pair_id).cam2_from_cam1.rotation.toRotationMatrix().transpose() * images[image_id2].gravity_info.GetRAlign()).col(1));
      } else {
          gravities.emplace_back((image_pairs.at(pair_id).cam2_from_cam1.rotation.toRotationMatrix() * images[image_id1].gravity_info.GetRAlign()).col(1));
      }

      ceres::CostFunction* coor_cost = GravError::CreateCost(gravities[counter]);
      problem.AddResidualBlock(coor_cost, options_.loss_function.get(), gravity.data());
      counter++;
    }

    if (gravities.size() < options_.min_num_neighbors)
        continue;
    ...
    for (int i = 0; i < gravities.size(); i++) {
      double error = RadToDeg(std::acos(std::max(std::min(gravities[i].dot(gravity), 1.), -1.)));
      if (error > options_.max_gravity_error * 2)
        counter_outlier++;
    }
    // If the refined gravity now consistent with more images, then accept it
    if (double(counter_outlier) / double(gravities.size()) < options_.max_outlier_ratio) {
      counter_rect++;
      images[image_id].gravity_info.SetGravity(gravity);
    }
  }
  std::cout << std::endl;
  LOG(INFO) << "Number of rectified images: " << counter_rect << " / " << error_prone_images.size() << std::endl;

}

void GravityRefiner::IdentifyErrorProneGravity(const ViewGraph& view_graph,
  ...
  for (const auto &[pair_id, image_pair] : view_graph.image_pairs) {
    ...
    if (image1.gravity_info.has_gravity && image2.gravity_info.has_gravity) {
      ...
      Eigen::Matrix3d R_rel = image2.gravity_info.GetRAlign().transpose() * image_pair.cam2_from_cam1.rotation.toRotationMatrix() * image1.gravity_info.GetRAlign();
  ...
  for (auto& [image_id, counter] : image_counter) {
    if (images.at(image_id).gravity_info.has_gravity == false)
      continue;
    if (counter.second < options_.min_num_neighbors)
      continue;
    if (double(counter.first) / double(counter.second) >= options_.max_outlier_ratio) {
      error_prone_images.insert(image_id);
    }
  }
  LOG(INFO) << "Number of error prone images: " << error_prone_images.size() << std::endl;
}
}  // namespace glomap
