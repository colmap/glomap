...
struct BATAPairwiseDirectionError {
    BATAPairwiseDirectionError(const Eigen::Vector3d& translation_obs): translation_obs_(translation_obs) {};

    // The error is given by the position error described above.
    template <typename T>
    bool operator() (const T* position1, const T* position2, const T* scale, T* residuals) const {
    ...
    static ceres::CostFunction* Create(const Eigen::Vector3d& translation_obs) {
        return (new ceres::AutoDiffCostFunction<BATAPairwiseDirectionError, 3, 3, 3, 1>(new BATAPairwiseDirectionError(translation_obs)));
    }
...
inline Eigen::Vector4d fetzer_d(const Eigen::Vector3d& ai, const Eigen::Vector3d& bi,
...
inline std::array<Eigen::Vector4d, 3> fetzer_ds(Eigen::Matrix3d& i1_G_i0) {
    Eigen::JacobiSVD<Eigen::Matrix3d> svd(i1_G_i0,
    ...
    Eigen::Vector3d bi =
        Eigen::Vector3d(s(0) * s(0) * v_0(2) * v_0(2),
                        s(0) * s(1) * v_0(2) * v_1(2),
                        s(1) * s(1) * v_1(2) * v_1(2));
...
public:
    ...
    static ceres::CostFunction* Create(const Eigen::Matrix3d i1_F_i0,
        ...
        return (new ceres::AutoDiffCostFunction<FetzerFocalLengthCost, 2, 1, 1>(
            new FetzerFocalLengthCost(i1_F_i0, principal_point0, principal_point1)));
    }
    ...
    bool operator()(const T* const fi_, const T* const fj_, T* residuals) const {
        ...
        T K0_01 = -(fj * fj * d_01_(2) + d_01_(3)) / (fj * fj * d_01_(0) + d_01_(1));
        T K1_12 = -(fi * fi * d_12_(1) + d_12_(3)) / (fi * fi * d_12_(0) + d_12_(2));
...
public:
    FetzerFocalLengthSameCameraCost(const Eigen::Matrix3d& i1_F_i0, const Eigen::Vector2d& principal_point) {
        Eigen::Matrix3d K0 = Eigen::Matrix3d::Identity(3, 3);
    ...
    static ceres::CostFunction* Create(const Eigen::Matrix3d i1_F_i0, const Eigen::Vector2d& principal_point) {
    return (new ceres::AutoDiffCostFunction<FetzerFocalLengthSameCameraCost, 2, 1>(
