...
// Code is adapted from Theia's RobustRotationEstimator (http://www.theia-sfm.org/)
// For gravity aligned rotation averaging, refere to the paper "Gravity Aligned Rotation Averaging"
namespace glomap {
...
struct ImagePairTempInfo {
    ...
    // angle_rel is the converted angle if gravity prior is available for both images
    double angle_rel = 0;
};
...
protected:
    ...
    // rotation magnitudes.
    double ComputeAverageStepSize(const std::unordered_map<image_t, Image>& images);
    ...
    // The fixed camera rotation (if with initialization, it would not be identity matrix)
