...
bool ViewGraphCalibrator::Solve(ViewGraph& view_graph,
    ...
    if (cameras.size() < 50)
        options_.solver_options.linear_solver_type
...
void ViewGraphCalibrator::Reset(std::unordered_map<camera_t, Camera>& cameras) {
    ...
    options_.loss_function = new ceres::CauchyLoss(options_.thres_loss_function);
}

void ViewGraphCalibrator::AddImagePairsToProblem(ViewGraph& view_graph,
                        std::unordered_map<camera_t, Camera>& cameras,
                        std::unordered_map<image_t, Image>& images) {
    for (auto &[image_pair_id, image_pair] : view_graph.image_pairs) {
        if (image_pair.config != 2 && image_pair.config != 3) 
            continue;
...
void ViewGraphCalibrator::AddImagePair(ImagePair& image_pair,
                        std::unordered_map<camera_t, Camera>& cameras,
                        std::unordered_map<image_t, Image>& images) {
    ...
    if (camera_id1 == camera_id2) {
        cost_function = FetzerFocalLengthSameCameraCost::Create(image_pair.F, cameras[camera_id1].PrincipalPoint());
        problem_->AddResidualBlock(cost_function,
                                options_.loss_function,
                                &(focals_[camera_id1]));
    } else {
        cost_function = FetzerFocalLengthCost::Create(image_pair.F, cameras[camera_id1].PrincipalPoint(), cameras[camera_id2].PrincipalPoint());
        problem_->AddResidualBlock(cost_function,
                                options_.loss_function,
                                &(focals_[camera_id1]),
                                &(focals_[camera_id2]));
...
size_t ViewGraphCalibrator::ParameterizeCameras(std::unordered_map<camera_t, Camera>& cameras) {
...
void ViewGraphCalibrator::ConvertResults(std::unordered_map<camera_t, Camera>& cameras) {
    size_t counter = 0;
    for (auto &[camera_id, camera] : cameras) {
        if (!problem_->HasParameterBlock(&(focals_[camera_id])))
            continue;
    ...
    std::cout << counter << " cameras are rejected in view graph calibration" << std::endl;
}

size_t ViewGraphCalibrator::FilterImagePair(ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images) {
    ...
    for (auto &[image_pair_id, image_pair] : view_graph.image_pairs) {
        ...
        Eigen::Vector2d error = Eigen::Vector2d(residuals[counter], residuals[counter+1]);
    ...
    std::cout << "invalid / total number of two view geometry: " << invalid_counter << " / " << counter << std::endl;
