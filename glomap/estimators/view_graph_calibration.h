...
struct ViewGraphCalibratorOptions : public OptimizationBaseOptions {
    // The minimal ratio of the estimated focal length against the prior focal length
    double thres_lower_ratio = 0.1;
    // The maximal ratio of the estimated focal length against the prior focal length
...
private:
    // Reset the problem
    void Reset(std::unordered_map<camera_t, Camera>& cameras);

    // Add the image pairs to the problem
    void AddImagePairsToProblem(ViewGraph& view_graph,
                        std::unordered_map<camera_t, Camera>& cameras,
                        std::unordered_map<image_t, Image>& images);

    // Add a single image pair to the problem
    void AddImagePair(ImagePair& image_pair,
                        std::unordered_map<camera_t, Camera>& cameras,
                        std::unordered_map<image_t, Image>& images);

    // Set the cameras to be constant if they have prior intrinsics
    size_t ParameterizeCameras(std::unordered_map<camera_t, Camera>& cameras);

    // Convert the results back to the camera
    void ConvertResults(std::unordered_map<camera_t, Camera>& cameras);

    // Filter the image pairs based on the calibration results
    size_t FilterImagePair(ViewGraph& view_graph,
                        std::unordered_map<image_t, Image>& images);
