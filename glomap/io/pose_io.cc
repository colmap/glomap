...
void ReadRelPose(const std::string& file_path,
    ...
    while (std::getline(file, line)) {
        ...
        if (name_idx.find(file1) == name_idx.end()) {
            max_image_id += 1;
            images.insert(std::make_pair(max_image_id, Image(max_image_id, -1, file1)));
            name_idx[file1] = max_image_id;
        }
        if (name_idx.find(file2) == name_idx.end()) {
            max_image_id += 1;
            images.insert(std::make_pair(max_image_id, Image(max_image_id, -1, file2)));
        ...
        view_graph.image_pairs.insert(std::make_pair(pair_id, ImagePair(index1, index2, pose_rel)));
        counter++;
    }
