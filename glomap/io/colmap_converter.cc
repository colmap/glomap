...
void ConvertGlomapToColmap(const std::unordered_map<camera_t, Camera>& cameras,
    ...
    if (tracks.size() > 0 || include_image_points) {
        ...
        for (auto &[image_id, image] : images) {
            if (!image.is_registered)
                continue;
            image_to_point3D[image_id] = std::vector<track_t>(image.features.size(), -1);
        }
    ...
    for (const auto &[track_id, track] : tracks) {
        ...
        // Add track element
        for (auto &observation : track.observations) {
            if (images.at(observation.first).is_registered == false)
                continue;
...
void ConvertColmapToGlomap(const colmap::Reconstruction& reconstruction,
    ...
    for (auto &[image_id, image_colmap] : reconstruction.Images()) {
        auto ite = images.insert(std::make_pair(image_colmap.ImageId(), Image(image_colmap.ImageId(), image_colmap.CameraId(), image_colmap.Name())));
...
void ConvertColmapPoints3DToGlomapTracks(
    ...
    for (auto &[point3d_id, point3D] : points3D) {
        ...
        for (auto &element : elements) {
            track.observations.push_back(Observation(element.image_id, element.point2D_idx));
        }
...
void ConvertDatabaseToGlomap(const std::string& database_path,
    ...
    for (auto &image : images_colmap) {
        ...
        if (image_id == colmap::kInvalidImageId)
            continue;
        auto ite = images.insert(std::make_pair(image_id, Image(image_id, image.CameraId(), image.Name())));
    ...
    for (auto &[image_id, image] : images) {
        ...
        for (int i = 0; i < keypoints.size(); i++) {
            image.features.emplace_back(Eigen::Vector2d(keypoints[i].x, keypoints[i].y));
        }
    }
    std::cout << "keypoints read done. " << images.size() << " images" << std::endl;
    ...
    // Add the matches
    std::vector<std::pair<colmap::image_pair_t, colmap::FeatureMatches>> all_matches = database.ReadAllMatches();

    // Go through all matches and store the matche with enough observations in the view_graph
    ...
    for (size_t match_idx = 0; match_idx < all_matches.size(); match_idx++) {
        ...
        std::pair<colmap::image_t, colmap::image_t> image_pair_colmap = database.PairIdToImagePair(pair_id);
        ...
        auto ite = image_pairs.insert(std::make_pair(ImagePair::ImagePairToPairId(image_id1, image_id2), ImagePair(image_id1, image_id2)));
        ImagePair& image_pair = ite.first->second;

        colmap::TwoViewGeometry two_view = database.ReadTwoViewGeometry(image_id1, image_id2);
        ...
        if (two_view.config == 3) {
            ...
            FundamentalFromMotionAndCameras(cameras.at(images.at(image_pair.image_id1).camera_id),
        ...
        std::vector<Eigen::Vector2d>& keypoints1 = images[image_pair.image_id1].features;
        std::vector<Eigen::Vector2d>& keypoints2 = images[image_pair.image_id2].features;
        ...
        for (int i = 0; i < feature_matches.size(); i++) {
            ...
            if (point2D_idx1 != colmap::kInvalidPoint2DIdx && point2D_idx2 != colmap::kInvalidPoint2DIdx) {

                if (keypoints1.size() <= point2D_idx1 || keypoints2.size() <= point2D_idx2)
                    continue;
    ...
    std::cout << "Pairs read done. " << view_graph.image_pairs.size() << " pairs. Number of invalid pairs: " << invalid_count << std::endl;
