...
bool RotationAverager::Solve(ViewGraph& view_graph,
    ...
    if (solve_1dof_system) {
        ...
        for (const auto& [pair_id, image_pair] : view_graph.image_pairs) {
            ...
            if (image1.gravity_info.has_gravity && image2.gravity_info.has_gravity) {
                view_graph_grav.image_pairs.emplace(pair_id, ImagePair(image_id1, image_id2, image_pair.cam2_from_cam1));
                grav_pairs++;
            }
        }
    }

    // If there is no image pairs with gravity or most image pairs are with gravity, then just run the 3dof version
    bool status = false;
    status = total_pairs || grav_pairs == 0;
    ...
    if (solve_1dof_system) {
        ...
        LOG(INFO) << "Solving subset 1DoF rotation averaging problem in the mixed prior system";
