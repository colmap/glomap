...
size_t TrackEngine::EstablishFullTracks(std::unordered_map<track_t, Track>& tracks) {
    tracks.clear();
    uf_.Clear();
...
void TrackEngine::BlindConcatenation() {
    // Initialize the union find data structure by connecting all the correspondences
    ...
    for (auto pair : view_graph_.image_pairs) {
        ...
        for (size_t i = 0; i < inliers.size(); i++) {
            ...
            image_pair_t point_global_id1 = 
                static_cast<image_pair_t>(image_pair.image_id1) << 32 | static_cast<image_pair_t>(point1_idx);
            image_pair_t point_global_id2 = 
                static_cast<image_pair_t>(image_pair.image_id2) << 32 | static_cast<image_pair_t>(point2_idx);

            // Link the first point to the second point. Take the smallest one as the root
...
void TrackEngine::TrackCollection(std::unordered_map<track_t, Track>& tracks) {
    ...
    size_t counter = 0;
    for (auto pair : view_graph_.image_pairs) {
        std::cout << "\r Establishing pairs " << counter + 1 << " / " << view_graph_.image_pairs.size() << std::flush;
        counter++;
    ...
    for (auto &[track_id, correspondence_set] : track_map) {
        if ((counter + 1) % 1000 == 0 || counter == track_map.size() - 1)
            std::cout << "\r Establishing tracks " << counter + 1 << " / " << track_map.size() << std::flush;
        counter++;
        ...
        for (auto point_global_id : correspondence_set) {
            ...
            if (image_id_set.find(image_id) != image_id_set.end()) {
                for (const auto& feature : image_id_set.at(image_id)) {
                    if ((feature - images_.at(image_id).features[feature_id]).norm() > options_.thres_inconsistency) {
                ...
                image_id_set.insert(std::make_pair(image_id, std::vector<Eigen::Vector2d>()));
            
            image_id_set[image_id].push_back(images_.at(image_id).features[feature_id]);
    ...
    std::cout << std::endl;
    std::cout << "Discarded " << discarded_counter << " tracks due to inconsistency" << std::endl;
}
...
size_t TrackEngine::FindTracksForProblem(
    ...
    for (const auto &[track_id, track] : tracks_full) {
        ...
        if (track.observations.size() > options_.max_num_view_per_track)
            continue;
        track_lengths.emplace_back(std::make_pair(track.observations.size(), track_id));
    }
    ...
    // If we only want to select a subset of images, then only add the tracks corresponding to those images
    ...
    for (const auto &[track_length, track_id] : track_lengths) {
        ...
        for (const auto &[image_id, feature_id] : track.observations) {
            ...
            track_temp.observations.emplace_back(std::make_pair(image_id, feature_id));
            image_ids.insert(image_id);
        }

        if (image_ids.size() < options_.min_num_view_per_track)
            continue;
        
        // A flag to see if the track has already been added or not to avoid multiple
        // insertion into the set to be efficient
        bool added = false;
        ...
        if (cameras_left == 0)
            break;
        if (tracks.size() > options_.max_num_tracks)
            break;

    }

    // To avoid flushing the track_full, we copy the selected tracks to the selected tracks
