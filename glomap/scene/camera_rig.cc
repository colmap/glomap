...
double CameraRig::ComputeRigFromWorldScale(
  ...
  for (const auto& snapshot : snapshots_) {
    for (size_t i = 0; i < num_cameras; ++i) {
        proj_centers_in_rig[i] = colmap::Inverse(CamFromRig(images[snapshot[i]].cam_from_world)).translation;
...
bool CameraRig::ComputeCamsFromRigs(const std::unordered_map<image_t, Image>& images) {
  THROW_CHECK_GT(NumSnapshots(), 0);
  ...
  for (const auto& snapshot : snapshots_) {
    ...
    //   const auto& image = reconstruction.Image(image_id);
      ...
      if (image.camera_id_ == ref_camera_id_) {
        ref_image = &image;
        break;
      }
    }
...
Rigid3d CameraRig::ComputeRigFromWorld(size_t snapshot_idx,
                              const std::unordered_map<image_t, Image>& images) {
