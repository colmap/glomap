...
image_pair_t ViewGraphManipulater::SparsifyGraph(
    ...
    // Here, the average is the harmonic mean of the degrees
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        if (images[image_id1].is_registered == false || images[image_id2].is_registered == false)
            continue;
        ...
        if (rand() / double(RAND_MAX) < (expected_degree * average_degree) / (degree1 * degree2)) {
            chosen_edges.insert(pair_id);
        }
    }
...
image_t ViewGraphManipulater::EstablishStrongClusters(
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        bool status = false;
        status = status || (criteria == INLIER_NUM && image_pair.inliers.size() > min_thres);
        status = status || (criteria == WEIGHT && image_pair.weight > min_thres);
        if (status) {
            uf.Unite(image_pair_t(image_pair.image_id1), image_pair_t(image_pair.image_id2));
        }
    }

    // For every two connected components, we check the number of slightly weaker pairs (> 0.75 min_thres) between them
    // Two clusters are concatenated if the number of such pairs is larger than a threshold (2)
    bool status = true;
    int iteration = 0;
    while (status) {
        status = false;
        ...
        std::unordered_map<image_pair_t, std::unordered_map<image_pair_t, int>> num_pairs;
        for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
            if (image_pair.is_valid == false)
                continue;
            ...
            bool status = false;
            status = status || (criteria == INLIER_NUM && image_pair.inliers.size() < 0.75 * min_thres);
            status = status || (criteria == WEIGHT && image_pair.weight < 0.75 * min_thres);
            if (status)
                continue;
            ...
            if (num_pairs.find(root1) == num_pairs.end())
                num_pairs.insert(std::make_pair(root1, std::unordered_map<image_pair_t, int>()));
            if (num_pairs.find(root2) == num_pairs.end())
                num_pairs.insert(std::make_pair(root2, std::unordered_map<image_pair_t, int>()));

            num_pairs[root1][root2]++;
            num_pairs[root2][root1]++;
        }
        // Connect the clusters progressively. If two clusters have more than 3 pairs, then connect them
...
void ViewGraphManipulater::UpdateImagePairsConfig(
    ...
    // For each camera, check the number of times that the camera is involved in a pair with configuration 2
    // First: the total occurence; second: the number of pairs with configuration 2
    ...
    // Check the ratio of valid and invalid relative pair, if the majority of the pairs are valid, then set the camera to valid
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        if (image_pair.is_valid == false)
            continue;
        if (image_pair.config != 3)
            continue;
        
        camera_t camera_id1 = images.at(image_pair.image_id1).camera_id;
        camera_t camera_id2 = images.at(image_pair.image_id2).camera_id;

        const Camera& camera1 = cameras.at(images.at(image_pair.image_id1).camera_id);
        const Camera& camera2 = cameras.at(images.at(image_pair.image_id2).camera_id);
        
        if (camera_validity[camera_id1] && camera_validity[camera_id2]) {
            image_pair.config = 2;
            FundamentalFromMotionAndCameras(camera1, camera2, image_pair.cam2_from_cam1, &image_pair.F);
        }
    }
}
...
void ViewGraphManipulater::DecomposeRelPose(ViewGraph& view_graph,
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        if (image_pair.is_valid == false)
            continue;
        // if (image_pair.config == 2)
        if (image_pair.config != 6)
            continue;
        if (!cameras[images[image_pair.image_id1].camera_id].has_prior_focal_length
        || !cameras[images[image_pair.image_id2].camera_id].has_prior_focal_length)
            continue;
        image_pair_ids.push_back(pair_id);
    }
    std::cout << "Decompose relative pose for " << image_pair_ids.size() << " pairs" << std::endl;
...
# pragma omp parallel for
    for (size_t idx = 0; idx < image_pair_ids.size(); idx++) {
        ...
        colmap::EstimateTwoViewGeometryPose(cameras[camera_id1], images[image_id1].features, cameras[camera_id2], images[image_id2].features, &two_view_geometry);

        // if it planar, then use the estimated relative pose 
        if (two_view_geometry.config == 4 && cameras[camera_id1].has_prior_focal_length && cameras[camera_id2].has_prior_focal_length) {
            image_pair.config = 2;
            continue;
        } else if (!(cameras[camera_id1].has_prior_focal_length && cameras[camera_id2].has_prior_focal_length))
            continue;
        ...
        if (image_pair.cam2_from_cam1.translation.norm() > EPS) {
            image_pair.cam2_from_cam1.translation = image_pair.cam2_from_cam1.translation.normalized();
        }
    ...
    for (size_t idx = 0; idx < image_pair_ids.size(); idx++) {
        ImagePair& image_pair = view_graph.image_pairs.at(image_pair_ids[idx]);
        if (image_pair.config != 2 && image_pair.config != 6)
            counter++;
    }
    std::cout << "Decompose relative pose done. " << counter << " pairs are pure rotation" << std::endl;
}
