...
void RelPoseFilter::FilterRotations(ViewGraph& view_graph,
    ...
    std::cout << "Filtered " << num_invalid << " relative rotation with angle > " << max_angle << " degrees" << std::endl;
}
...
void RelPoseFilter::FilterInlierNum(ViewGraph& view_graph,
    ...
    std::cout << "Filtered " << num_invalid << " relative poses with inlier number < " << min_inlier_num << std::endl;
}
...
void RelPoseFilter::FilterInlierRatio(ViewGraph& view_graph,
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        ...
        if (image_pair.inliers.size() / double(image_pair.matches.rows()) < min_inlier_ratio) {
            image_pair.is_valid = false;
            num_invalid++;
        }
    }

    std::cout << "Filtered " << num_invalid << " relative poses with inlier ratio < " << min_inlier_ratio << std::endl;
}
