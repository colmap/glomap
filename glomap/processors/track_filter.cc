...
int TrackFilter::FilterTracksByReprojection(
    ...
    for (auto &[track_id, track] : tracks) {
        ...
        for (auto &[image_id, feature_id] : track.observations) {
            ...
            if (in_normalized_image) {
                const Eigen::Vector3d& feature_undist = image.features_undist.at(feature_id);

                Eigen::Vector2d pt_reproj = pt_calc.head(2) / pt_calc(2);
                reprojection_error = (pt_reproj - feature_undist.head(2) / (feature_undist(2) + EPS)).norm();
            } else {
                Eigen::Vector2d pt_reproj = pt_calc.head(2) / pt_calc(2);
    ...
    std::cout << "Filtered " << counter << " / " << tracks.size() << " tracks by reprojection error" << std::endl;
    return counter;
}
...
int TrackFilter::FilterTracksByAngle(
    ...
    for (auto &[track_id, track] : tracks) {
        ...
        for (auto &[image_id, feature_id] : track.observations) {
            ...
            double thres_cam = (cameras.at(image.camera_id).has_prior_focal_length) ? thres : thres_uncalib;
    ...
    std::cout << "Filtered " << counter << " / " << tracks.size() << " tracks by angle error" << std::endl;
    return counter;
}
...
int TrackFilter::FilterTrackTriangulationAngle(
    ...
    std::cout << "Filtered " << counter << " / " << tracks.size() << " tracks by too small triangulation angle" << std::endl;
    return counter;
}
