...
double ImagePairInliers::ScoreError() {
    ...
    if (image_pair.config == 6 || image_pair.config == 4 || image_pair.config == 5) 
...
double ImagePairInliers::ScoreErrorEssential() {
    ...
    for (size_t k = 0; k < image_pair.matches.rows(); ++k) {
        ...
        if (r2 < sq_threshold) {
            ...
            // Check whether two image rays have small triangulation angle or are too close to the epipoles
            ...
            not_denegerate = not_denegerate && (diff_epipole1 < thres_epipole && diff_epipole2 < thres_epipole);
...
double ImagePairInliers::ScoreErrorFundamental() {
    ...
    double sq_threshold = options.max_epipolar_error_F * options.max_epipolar_error_F;
    double score = 0.;
    ...
    // Then, if the signum is not consistent with the cheirality, discard the point
...
double ImagePairInliers::ScoreErrorHomography() {
    ...
    double sq_threshold = options.max_epipolar_error_H * options.max_epipolar_error_H;
    double score = 0.;
    Eigen::Vector2d pt1, pt2;
    for (size_t k = 0; k < image_pair.matches.rows(); ++k) {
        pt1 = images.at(image_id1).features[image_pair.matches(k, 0)];
...
void ImagePairsInlierCount(ViewGraph& view_graph,
    ...
    for (auto &[pair_id, image_pair] : view_graph.image_pairs) {
        if (!clean_inliers && image_pair.inliers.size() > 0)
            continue;
        image_pair.inliers.clear();
